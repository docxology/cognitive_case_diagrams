...
![Born-rule probabilities for orthogonal projectors and the synthetic diagonal mixture, with computed outcome probabilities ${quantum_nom_probability} on NOM, ${quantum_acc_probability} on ACC, and ${quantum_dat_probability} on DAT. Remaining outcomes have zero probability. These bars are computed traces; they show neither interference fringes nor a continuous semantic-state density.](output/figures/quantum_povm_probabilities.png){#fig:quantum-povm}
