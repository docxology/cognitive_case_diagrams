...
| **Quantum magnitude homology** | The §5b theoretical target: magnitude homology corrected for quantum decoherence, $\|\mathcal{C}\|_q = \|\mathcal{C}\|(1-\lambda)$ (LM-enriched magnitude homology in Bradley and Vigneaux 2025). **Not implemented** — `compute_pqc_decoherence_proxy()` returns only a coarse decoherence proxy (real syntactic complexity, a `cups − caps` hole count, and `min(1.0, noise·1.5^holes)` vs an unsourced 0.25 threshold) | `compute_pqc_decoherence_proxy()` | §5b |
