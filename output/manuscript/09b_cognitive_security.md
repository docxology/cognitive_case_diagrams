...
![Policy cell matrix for the supplied role-separation rules: each cell records whether role separation and the required edge hold under this rule. A failed cell records a label merge or missing edge under this rule, not mathematical failure of a monoidal law and not evidence that a linguistic alignment is malicious.](output/figures/monoidal_functor_security.png){#fig:monoidal-functor-security}
