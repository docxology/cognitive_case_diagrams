...
\begin{frame}[allowframebreaks]{\([0,1]\)-Enriched Case Categories:
Hom-Values as Distributional Proximity}
...
\begin{frame}[allowframebreaks]{Why Binary Morphisms Are Not Enough}
...
\begin{frame}[fragile,allowframebreaks]{Enriching Over
\(([0,1],\cdot,1)\): Identity, Sub-Multiplicative Composition, and Four
Hom-Value Readings}
\protect\phantomsection\label{enriching-over-01cdot1-identity-sub-multiplicative-composition-and-four-hom-value-readings}
\begin{block}{The Identity and Composition Axioms for \([0,1]\)-Enriched
Case Categories}
\protect\phantomsection\label{the-identity-and-composition-axioms-for-01-enriched-case-categories}
...
\begin{block}{Four Linguistic Readings of the Hom-Value: Probability,
Proto-Role, Similarity, Predictability}
\protect\phantomsection\label{four-linguistic-readings-of-the-hom-value-probability-proto-role-similarity-predictability}
...
\begin{block}{When the Composition Inequality Fails: A Worked English
NOM--ACC--DAT Example}
\protect\phantomsection\label{when-the-composition-inequality-fails-a-worked-english-nomaccdat-example}
...
\item
  ...
  \emph{čeloveka}) ⇒ \(\mathcal{C}(\text{ACC}, \text{GEN}) \approx 1\)
  for that declension class.
\item
  \textbf{Russian neuter NOM = ACC} (e.g.~\emph{okno} ``window''
  identical in subject and direct-object position) ⇒
...
\item
  ...
  ``toward-DAT the city'' and \emph{u gradu} ``in-LOC the city'') ⇒
