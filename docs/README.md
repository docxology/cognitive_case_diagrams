...
| **Anyone** looking up a term | [`glossary.md`](glossary.md) → term rows across math, linguistics, distributional RL, and code |
...
| [glossary.md](glossary.md) | Term rows: math ↔ linguistics ↔ code ↔ distributional RL |
...
| `visualization/` | all | Publication figures (300 DPI, 16pt min font) |
...
Canonical counts. `output/metrics.json` is regenerated by `src/generate_manuscript_metrics.py` and is authoritative wherever it is cited below; the snapshot values are convenience only and may lag a regeneration.

| Metric | Count | Source of Truth |
|--------|-------|-----------------|
| Manuscript sections | 24 `.md` section files | `docs/manuscript/` (live count: `ls docs/manuscript/[0-9]*.md \| wc -l`) |
| Publication figures | **30** PNGs | `output/figures/` + `docs/manuscript_figure_index.md`; authoritative live count in `output/metrics.json::total_figures` |
| BibTeX entries | **106** keys | `docs/manuscript/references.bib` (live count: `grep -c '^@' docs/manuscript/references.bib`) |
| Total tests | see source of truth | `output/metrics.json::total_test_count` (live count: `uv run pytest tests/ --collect-only -q`); regenerate before quoting |
...
| DAIF tests | see source of truth | `output/metrics.json::daif_tests` + `daif_test_files` (`tests/test_daif*.py`); regenerate before quoting |
...
| `src/` line + branch coverage | see source of truth | `output/metrics.json::coverage_percent` (with `coverage_lines_covered` / `coverage_branches_covered`); regenerate before quoting |
| Glossary terms | see source of truth | `docs/glossary.md` — bolded term rows (live count: `grep -c '^\| \*\*' docs/glossary.md`) |
| ADR count | 12 | `docs/AGENTS.md` (ADR-001 through ADR-012) |
| `\autoref{}` cross-references | see source of truth | `docs/manuscript/*.md` (live count: `grep -o '\\autoref{' docs/manuscript/*.md \| wc -l`) |
