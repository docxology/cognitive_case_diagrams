...
A related commitment appears at corpus scale in recent work on causal discourse extraction. Democritus groups paraphrase families of extracted causal claims into homotopy-localized classes, so that restated variants of one claim do not multiply the evidence count [@mahadevan2026democritus]. The present chapter's entity bookkeeping makes an analogous commitment at the opposite scale: role histories are recorded only for explicitly supplied names, sentence by sentence, and repeated spellings do not by themselves assert coreference. In both cases surface variation is treated as a modeling input rather than as new evidence — here by keeping each entity's history explicit, there by aggregating paraphrase families into localized claim classes. The shared boundary is also the same: neither layer claims semantic correctness of its identifications without an independent annotation layer, and neither supplies one.
