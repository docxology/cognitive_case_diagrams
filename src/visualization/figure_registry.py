...
- entries carry exactly ``filename, path, label, alt_text, generated_by,
  generator_input_fingerprint, environment_fingerprint, sha256, provenance``
  in that key order,
...
- ``environment_fingerprint`` binds each entry to the rendering environment
  (python, numpy, matplotlib, discopy versions) so committed figures stay
  reproducible; ``src.project_validation`` raises the fail-closed
  ``Stale figure generation environment: <figure>`` diagnostic when the live
  environment disagrees, and likewise for entries missing the field.
...
def _distribution_version(name: str) -> str:
    """Return the installed distribution version or a stable sentinel.

    Resolved the same way ``generate_manuscript_metrics`` resolves optional
    distributions: a package that is not installed contributes the stable
    sentinel ``absent`` rather than silently varying between runs.
    """
...
def environment_fingerprint() -> str:
    """Deterministic digest binding figures to their rendering environment.

    sha256 over the sorted ``name==version`` lines for python (first token of
    ``sys.version``) and the numpy, matplotlib, and discopy distributions.
    Written into each registry entry as ``environment_fingerprint``;
    ``src.project_validation`` rejects committed figures whose recorded
    fingerprint no longer matches the live environment.
    """
    import sys
    ...
    lines.extend(
        ...
        for name in sorted(("numpy", "matplotlib", "discopy"))
    )
...
def write_figure_registry(
    ...
    *,
    ...
    Each new entry records ``environment_fingerprint``, the deterministic
    digest of the rendering environment from :func:`environment_fingerprint`.
    ``src.project_validation`` fails closed with
    ``Stale figure generation environment: <figure>`` when a committed
    entry's recorded fingerprint differs from the live environment — or when
    the entry lacks the field (schema evolution is intentional; entries are
    regenerated by the pipeline). The source-bytes
    ``generator_input_fingerprint`` check is unchanged.
    """
