...
A complementary conditional-grounding pattern appears in the causal-discourse setting. Democritus states a relative grounding functor from localized discourse to observational-equivalence classes of causal models as a conditional proposition, with its hypotheses — cocompleteness and density conditions among them — left explicitly to check rather than asserted [@mahadevan2026democritus]; the work forms part of a broader categorical-AI research program developed in dated draft editions [@mahadevan2026categories]. This matches the present chapter's posture. `bridge_transfer()` likewise reports an unknown equivalence and performs no transfer: in both cases the topos and bridge language functions as a specification of what evidence a real grounding or transfer would require, with the missing evidence named rather than assumed.
